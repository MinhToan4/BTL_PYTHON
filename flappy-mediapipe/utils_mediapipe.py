...
class MediaPipeHand:
    def __init__(self, static_image_mode=True, max_num_hands=1):
        ...
        # self.param[i]['keypt'] /= np.array([img_width, img_height])[:, np.newaxis]

        # # Simple smoothing (average 3 frames)
        # if hasattr(self, 'prev_keypt'):  # Lưu prev
        #     self.param[i]['keypt'] = 0.7 * self.param[i]['keypt'] + 0.3 * self.prev_keypt
        self.prev_keypt = self.param[i]['keypt'].copy()
