"""
Tổng quan về file main_menu.py:

File main_menu.py định nghĩa class MainMenu, là hệ thống giao diện người dùng chính của game Race Master 3D.
Class này quản lý tất cả các menu trong game như menu khởi động, menu chính, menu chọn đường đua, menu cài đặt, menu garage, menu multiplayer, và menu tạm dừng.
Nó sử dụng Ursina UI để tạo giao diện 2D trên camera.ui, với các button, text, và animation.
Cấu trúc chính bao gồm: khởi tạo các menu con, tạo UI elements cho từng menu, và các hàm xử lý sự kiện click.
"""

# ==================== IMPORTS VÀ CẤU HÌNH BAN ĐẦU ====================
# File main_menu.py - Hệ thống giao diện menu chính của game Race Master 3D
# Đây là file quản lý toàn bộ UI (User Interface) của game, bao gồm:
# - Menu khởi động (Start Menu)
# - Menu chính (Main Menu)
# - Menu chọn đường đua (Maps Menu)
# - Menu cài đặt (Settings Menu)
# - Menu garage (Garage Menu)
# - Menu multiplayer (Multiplayer Menu)
# - Menu tạm dừng (Pause Menu)

from ursina import *  # Import toàn bộ Ursina Engine - framework 3D game
from ursina import curve  # Import curve để tạo animation mượt mà
from server import Server  # Import class Server để xử lý multiplayer
import os  # Import os để thao tác với hệ thống file

# ==================== CẤU HÌNH TEXT ====================
# Thiết lập độ phân giải mặc định cho tất cả Text objects trong game
# Điều này đảm bảo text hiển thị rõ nét trên mọi màn hình
Text.default_resolution = 1080 * Text.size

# ==================== CLASS MAINMENU ====================
# Class MainMenu kế thừa từ Entity của Ursina
# Đây là class chính quản lý toàn bộ giao diện menu của game
# Nó chứa tất cả các menu con và logic điều hướng giữa chúng
class MainMenu(Entity):
    # ==================== HÀM KHỞI TẠO ====================
    # Hàm __init__ được gọi khi tạo instance của MainMenu
    # Nó nhận các tham số: car (đối tượng xe), ai_list (danh sách AI), và tất cả các track objects
    def __init__(self, car, ai_list, sand_track, grass_track, snow_track, forest_track, savannah_track, lake_track):
        # Gọi hàm khởi tạo của class cha (Entity)
        # parent = camera.ui nghĩa là menu sẽ hiển thị trên UI camera (giao diện 2D)
        super().__init__(
            parent = camera.ui
        )

        # ==================== KHỞI TẠO CÁC MENU CON ====================
        # Tạo các Entity đại diện cho từng menu khác nhau
        # Mỗi menu là một container chứa các UI elements (buttons, text, etc.)
        # Chỉ có một menu được enable tại một thời điểm

        # Menu khởi động - menu đầu tiên người chơi thấy
        self.start_menu = Entity(parent = self, enabled = True)

        # Menu host server - để tạo server multiplayer
        self.host_menu = Entity(parent = self, enabled = False)

        # Menu hiển thị sau khi tạo server thành công
        self.created_server_menu = Entity(parent = self, enabled = False)

        # Menu tham gia server - để join vào server multiplayer
        self.server_menu = Entity(parent = self, enabled = False)

        # Menu chính - menu sau khi chọn chế độ chơi
        self.main_menu = Entity(parent = self, enabled = False)

        # Menu chọn chế độ đua (race, time trial, drift)
        ...
        # ==================== LƯU THAM CHIẾU ĐẾN CÁC OBJECT CHÍNH ====================
        # Lưu tham chiếu đến car object để điều khiển xe trong menu
        self.car = car

        # Lưu tham chiếu đến tất cả track objects
        ...
        # Lưu tham chiếu đến danh sách AI cars
        self.ai_list = ai_list

        # Biến để lưu tham chiếu đến sun object (cho lighting)
        self.sun = None

        # ==================== ÂM THANH ====================
        # Tạo Audio object cho âm thanh click khi nhấn button
        # volume = 10 nghĩa là âm lượng gấp 10 lần mặc định
        self.click = Audio("click.wav", False, False, volume = 10)

        # ==================== DANH SÁCH TRACKS ====================
        # Tạo list chứa tất cả track objects để dễ quản lý
        # Sử dụng trong vòng lặp để enable/disable tracks
        self.tracks = [
            self.sand_track, self.grass_track, self.snow_track, self.forest_track, self.savannah_track, self.lake_track
        ]

        # ==================== ANIMATION CHO MENU ====================
        # Thiết lập animation khi menu được enable
        # Tạo hiệu ứng fade in và scale cho các UI elements
        for menu in (self.start_menu, self.main_menu, self.race_menu, self.maps_menu, self.settings_menu, self.video_menu, self.gameplay_menu, self.audio_menu, self.controls_menu, self.pause_menu, self.quit_menu, self.garage_menu):
            # Tạo hàm animate_in_menu cho mỗi menu
            def animate_in_menu(menu = menu):
                # Lặp qua tất cả children (các button, text) của menu
                for i, e in enumerate(menu.children):
                    # Lưu scale gốc để animate về
                    e.original_scale = e.scale

                    # Giảm scale ban đầu để tạo hiệu ứng zoom in
                    e.scale -= 0.01

                    # Animate scale về original_scale với delay tăng dần
                    # delay = i * 0.05 tạo hiệu ứng cascade (tuần tự)
                    # duration = 0.1 là thời gian animation
                    # curve = curve.out_quad tạo easing mượt mà
                    e.animate_scale(e.original_scale, delay = i * 0.05, duration = 0.1, curve = curve.out_quad)

                    # Animate alpha (độ trong suốt) từ 0 lên 0.7
                    e.alpha = 0
                    e.animate("alpha", 0.7, delay = i * 0.05, duration = 0.1, curve = curve.out_quad)

                    # Nếu element có text_entity (như Button), animate text riêng
                    if hasattr(e, "text_entity"):
                        e.text_entity.alpha = 0
                        e.text_entity.animate("alpha", 1, delay = i * 0.05, duration = 0.1)

            # Gán hàm animate_in_menu vào event on_enable của menu
            # Khi menu được enable, animation sẽ tự động chạy
            menu.on_enable = animate_in_menu

        # ==================== START MENU ====================
        # Menu khởi động - menu đầu tiên người chơi thấy khi mở game
        # Hiển thị logo game và các lựa chọn: Singleplayer, Multiplayer, Quit

        # Đặt vị trí xe trong Start Menu để hiển thị như preview
        self.car.position = (-80, -42, 18.8)
        self.car.visible = True

        # Enable grass track làm background cho start menu
        ...
        # ==================== HÀM XỬ LÝ SINGLEPLAYER ====================
        # Hàm được gọi khi người chơi chọn chế độ chơi đơn
        def singleplayer():
            # Tắt chế độ multiplayer
            car.multiplayer = False

            # Chuyển từ start_menu sang main_menu
            self.start_menu.disable()
            self.main_menu.enable()

            # Đặt lại vị trí xe và ẩn nó
            grass_track.enable()
            self.car.position = (0, 0, 4)
            self.car.visible = False

            # Disable tất cả tracks và chỉ enable grass_track
        ...
        # ==================== HÀM XỬ LÝ MULTIPLAYER ====================
        # Hàm được gọi khi người chơi chọn chế độ multiplayer
        def multiplayer():
            # Chuyển từ start_menu sang host_menu
            self.start_menu.disable()
            self.host_menu.enable()

            # Hiển thị xe và chuyển sang snow track background
            self.car.visible = True
            self.car.position = (-3, -44.5, 92)
            grass_track.disable()
            snow_track.enable()

            # Disable tất cả tracks và enable snow_track
            for track in self.tracks:
                for i in track.track:
                    i.disable()
                for i in track.details:
        ...
        def quit():
            # Thoát ứng dụng Ursina
            application.quit()
            # Thoát process Python (đảm bảo thoát hoàn toàn)
            os._exit(0)

        # ==================== TẠO UI ELEMENTS CHO START MENU ====================
        # Tạo logo game ở trên cùng
        start_title = Entity(model = "quad", scale = (0.5, 0.2, 0.2), texture = "rally-logo", parent = self.start_menu, y = 0.3)

        # Tạo buttons cho Singleplayer và Multiplayer
        ...
        # Gán functions vào on_click events của buttons
        singleplayer_button.on_click = Func(singleplayer)
        multiplayer_button.on_click = Func(multiplayer)

        # ==================== QUIT MENU ====================
        # Menu xác nhận thoát game - hiển thị khi người chơi nhấn ESC ở Start Menu

        # Hàm xử lý khi người chơi chọn "No" (không thoát)
        def dont_quit():
            # Tắt quit_menu và enable start_menu
            self.quit_menu.disable()
            self.start_menu.enable()

        # Tạo UI elements cho quit menu
        ...
        def create_server():
            ...
            if str(self.car.host_ip.text) != "IP" and str(self.car.host_port) != "PORT":
                ...
                # Disable tất cả tracks và enable sand_track
                for track in self.tracks:
                    for i in track.track:
                        i.disable()
                    for i in track.details:
        ...
        # Hàm chuyển sang menu join server
        ...
        # ==================== TẠO UI ELEMENTS CHO HOST MENU ====================
        # Tạo InputField để nhập IP và PORT
        ...
        def flip_steering_settings():
            self.car.flip_steering = not self.car.flip_steering
            flip_steering_button.text = f"Flip Steering: {'ON' if self.car.flip_steering else 'OFF'}"
            self.car.save_unlocked()  # Lưu setting ngay lập tức
        ...
        flip_steering_button = Button(f"Flip Steering: {'ON' if self.car.flip_steering else 'OFF'}", color = color.black, scale_y = 0.1, scale_x = 0.3, y = -0.1, parent = self.controls_menu)
        ...
        flip_steering_button.on_click = Func(flip_steering_settings)
        ...
        def rally_hover():
            self.garage_name_text.enable()
            self.garage_name_text.text = "Racing Car"
        ...
        self.garage_name_text = Text("Racing Car", scale = 1.5, color = color.white, line_height = 2, origin = 0, x = -0.5, y = -0.4, parent = self.garage_menu)
    ...
    def garage_locked_text(self, warning):
        # ==================== HÀM HIỂN THỊ THÔNG BÁO KHÓA GARAGE ====================
        # Hàm này được gọi khi người chơi cố gắng unlock một item chưa đủ điều kiện
        # Nó hiển thị thông báo lỗi với hiệu ứng rung (shake) và tự động tắt sau 1 giây
        
        # Enable text hiển thị warning
        self.garage_unlocked_text.enable()
        
        # Đặt nội dung warning text
        self.garage_unlocked_text.text = warning
        
        # Tạo hiệu ứng rung để thu hút sự chú ý
        self.garage_unlocked_text.shake()
        
        # Tự động disable text sau 1 giây bằng invoke()
        invoke(self.garage_unlocked_text.disable, delay = 1)

    def update(self):
        # ==================== HÀM UPDATE - ĐƯỢC GỌI MỖI FRAME ====================
        # Hàm này xử lý logic cập nhật liên tục cho menu system
        # Bao gồm camera control, AI slider, username saving, và leaderboard
        
        # ==================== CAMERA FOLLOW LOGIC ====================
        # Nếu không có menu nào được enable (đang trong game), bật camera follow
        # Camera follow giúp camera theo dõi xe trong game
        ...
        # ==================== AI SLIDER LOGIC ====================
        # Xử lý số lượng AI cars dựa trên giá trị slider (chỉ trong singleplayer)
        ...
        # ==================== CAMERA POSITIONING ====================
        # Điều khiển vị trí camera dựa trên menu hiện tại
        if self.start_menu.enabled or self.host_menu.enabled or self.garage_menu.enabled or self.server_menu.enabled or self.quit_menu.enabled:
            # Trong menu: cho phép xoay xe bằng chuột phải hoặc tự động xoay
            if not held_keys["right mouse"]:
                if self.start_spin:
                    # Tự động xoay xe để hiển thị đẹp
                    self.car.rotation_y += 15 * time.dt
            else:
                # Xoay xe theo vị trí chuột X
                self.car.rotation_y = mouse.x * 200

            # Lặp vị trí camera về vị trí xe + offset
            camera.position = lerp(camera.position, self.car.position + self.car.camera_offset, time.dt * self.car.camera_speed)

            # Điều chỉnh camera rotation và offset dựa trên menu
            ...
            # Trong game: camera follow xe với góc nhìn từ trên
        ...
        # ==================== USERNAME SAVING ====================
        # Tự động lưu username khi ở trong multiplayer menus
        ...
        # ==================== LEADERBOARD LOGIC ====================
        # Hiển thị leaderboard trong multiplayer mode
        if self.car.multiplayer_update:
            for menu in self.menus:
                if menu.enabled == False:
                    # Nếu đang trong game (không có menu nào enable)
                    if self.sand_track.enabled or self.grass_track.enabled or self.snow_track.enabled or self.forest_track.enabled or self.savannah_track.enabled:
                        # Hiển thị leaderboard sau delay 0.1 giây
                        invoke(self.start_leaderboard, delay = 0.1)
                else:
                    # Nếu có menu enable, tắt leaderboard
                    for l in self.leaderboard_texts:
                        l.disable()
        else:
            # Không phải multiplayer, tắt leaderboard
            for l in self.leaderboard_texts:
                l.disable()
            
    def start_leaderboard(self):
        # ==================== HÀM HIỂN THỊ LEADERBOARD ====================
        # Hàm này được gọi để hiển thị bảng xếp hạng multiplayer
        # Cập nhật text của các vị trí leaderboard từ dữ liệu car
        
        # Enable tất cả các text elements của leaderboard
        for l in self.leaderboard_texts:
            l.enable()

        # Cập nhật nội dung cho từng vị trí
    ...
    def input(self, key):
        # ==================== HÀM XỬ LÝ INPUT - ĐƯỢC GỌI KHI CÓ PHÍM BẤM ====================
        # Hàm này xử lý tất cả input từ keyboard và mouse
        # Bao gồm pause menu, audio, quit menu, settings navigation
        
        # ==================== PAUSE MENU LOGIC ====================
        # Xử lý phím ESC để toggle pause menu khi đang trong game
        if not self.start_menu.enabled and not self.main_menu.enabled and not self.server_menu.enabled and not self.settings_menu.enabled and not self.race_menu.enabled and not self.maps_menu.enabled and not self.settings_menu.enabled and not self.garage_menu.enabled and not self.audio_menu.enabled and not self.controls_menu.enabled and not self.host_menu.enabled and not self.created_server_menu.enabled and not self.video_menu.enabled and not self.gameplay_menu.enabled and not self.quit_menu.enabled:
            if key == "escape":
                # Toggle pause menu và lock/unlock mouse
                self.pause_menu.enabled = not self.pause_menu.enabled
                mouse.locked = not mouse.locked

            # Dừng animation xoay xe khi trong game
            self.start_spin = False

            # Xử lý timer và UI elements dựa trên trạng thái
            ...
            # Khi ở trong menu: disable tất cả game UI và timer
            self.car.timer.disable()
            self.car.reset_count_timer.disable()
        ...
        # ==================== AUDIO LOGIC ====================
        # Phát âm thanh click khi nhấn chuột trái vào button
        if self.car.audio:
            for menu in self.menus:
                if menu.enabled:
                    for i, e in enumerate(menu.children):
                        if e.hovered and key == "left mouse down":
                            # Điều chỉnh volume và phát âm thanh
                            self.click.volume = self.car.volume * 5
                            self.click.play()
        
        # Cập nhật volume từ slider trong audio menu
        if self.audio_menu.enabled:
            self.car.volume = self.volume.value

        # ==================== QUIT MENU LOGIC ====================
        # Xử lý phím ESC trong start menu để toggle quit confirmation
        if self.start_menu.enabled or self.quit_menu.enabled:
            if key == "escape":
        ...
        # Xử lý phím ESC để navigate back trong settings submenus
        if key == "escape":
        ...
        # ==================== CAR NORMALS LOGIC ====================
        # Điều khiển thuộc tính copy_normals của xe dựa trên trạng thái spin
        if self.start_spin:
